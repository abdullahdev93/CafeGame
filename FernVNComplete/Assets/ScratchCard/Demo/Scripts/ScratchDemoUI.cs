...
namespace ScratchCardAsset.Demo
{
	public class ScratchDemoUI : MonoBehaviour
	{
		...
		{
			var worldPosition = cardManager.Card.SurfaceTransform.localToWorldMatrix.MultiplyPoint(position);
		...
		{
			var worldPosition = cardManager.Card.SurfaceTransform.localToWorldMatrix.MultiplyPoint(lineEnd);
			cursor.transform.position = worldPosition;
		}
