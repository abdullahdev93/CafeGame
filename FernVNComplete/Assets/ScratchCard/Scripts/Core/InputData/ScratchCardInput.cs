...
namespace ScratchCardAsset.Core.InputData
{
	...
	public class ScratchCardInput
	{
		...
		{
#if ENABLE_INPUT_SYSTEM
			if ((inputMethods & InputMethods.Pen) == InputMethods.Pen && Pen.current != null && (Pen.current.press.isPressed || Pen.current.press.wasReleasedThisFrame))
			{
				if (Pen.current.press.isPressed)
				{
					...
					if (!Pen.current.press.wasPressedThisFrame)
					{
						SetInputData(InputMethods.Pen, 0, position, pressure);
					}
				}
			...
			else if ((inputMethods & InputMethods.Touch) == InputMethods.Touch && Touchscreen.current != null && Touch.activeTouches.Count > 0)
			{
				foreach (var touch in Touch.activeTouches)
				{
					...
					{
						SetInputData(InputMethods.Touch, fingerId, touch.screenPosition, pressure);
					}
			...
			else if ((inputMethods & InputMethods.Mouse) == InputMethods.Mouse && Mouse.current != null)
			{
				...
				{
					SetInputData(InputMethods.Mouse, 0, Mouse.current.position.ReadValue());
				}
