...
namespace ScratchCardAsset.Core.ScratchData
{
    public class Triangle
    {
        ...
        public Vector3 GetLocalPosition(Vector2 texturePosition, Vector2 textureSize)
        {
            var uv = texturePosition / textureSize;
            var t = uv1 - uv0;
            var u = uv2 - uv0;
            var v = uv - uv0;
            var denom = t.x * u.y - t.y * u.x;
            var a2 = (v.x * u.y - v.y * u.x) / denom;
            var a3 = (t.x * v.y - t.y * v.x) / denom;
            var a1 = 1 - a2 - a3;
            return v0 * a1 + v1 * a2 + v2 * a3;
        }
    }
}