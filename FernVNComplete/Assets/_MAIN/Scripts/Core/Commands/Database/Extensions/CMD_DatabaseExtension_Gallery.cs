...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Gallery : CMD_DatabaseExtension
    {
        ...
        new public static void Extend(CommandDatabase database)
        {
            ...
            database.AddCommand("UnlockCutscene", new Func<string[], IEnumerator>(UnlockGalleryVideo));
        }
        ...
        public static IEnumerator ShowGalleryImage(string[] data)
        {
            ...
            if (graphic == null)
            {
                Debug.LogError($"Could not find gallery image called '{mediaName}' in the Resources '{FilePaths.resources_gallery}' directory. Please specify the full path within resources and make sure that the file exists!");
                yield break;
            }
        ...
        public static IEnumerator UnlockGalleryVideo(string[] data)
        {
            string mediaName = "";
            ...
            {
                Debug.LogError("[unlockgalleryvideo] Missing -m <videoName> parameter.");
                yield break;
            }

            // Unlock by the clip name (this MUST equal VideoClip.name used in GalleryMenu)
            GalleryConfig.UnlockVideo(mediaName);
            Debug.Log($"[unlockgalleryvideo] Unlocked video '{mediaName}'.");

            // No visual work to do here; this is purely progression-state.
            yield break;
        } 
    }
}