...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_GraphicPanels : CMD_DatabaseExtension
    {
        ...
        private static IEnumerator SetLayerMedia(string[] data)
        {
            ...
            // Alias resolution: if media is "Apartment_ALT", pick the variant by Activity Phase
            if (!string.IsNullOrEmpty(mediaName) && string.Equals(mediaName, "Apartment_ALT", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    string phase = CalendarSystem.Instance.activityPhases[CalendarSystem.Instance.activityCounter];
                    switch (phase)
                    {
                        case "Morning":
                            mediaName = "Apartment_Background_M"; 
                            break;
                        case "Afternoon":
                            mediaName = "Apartment_Background_A";
                            break;
                        case "Evening":
                            mediaName = "Apartment_Background_E";
                            break;
                        default:
                            // Fallback if a new phase label appears
                            mediaName = "ApartmentM";
                            Debug.LogWarning($"[SetLayerMedia] Unrecognized phase '{phase}'. Falling back to '{mediaName}'.");
                            break;
                    }
                    Debug.Log($"[SetLayerMedia] Resolved 'Apartment_ALT' to '{mediaName}' for phase '{phase}'.");
                }
                catch (Exception ex)
                {
                    // If CalendarSystem isn't ready, fail gracefully to a sensible default
                    mediaName = "ApartmentM";
                    Debug.LogWarning($"[SetLayerMedia] Could not resolve 'Apartment_ALT' by phase (CalendarSystem not ready?). Using '{mediaName}'. Exception: {ex.Message}");
                }
            } 
