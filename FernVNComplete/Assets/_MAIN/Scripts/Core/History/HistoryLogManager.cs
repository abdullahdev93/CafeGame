...
namespace History
{
    public class HistoryLogManager : MonoBehaviour
    {
        ...
        private const string AUDIO_FROM_TEXT = "AudioFromText";
        private const string NO_AUDIO_TEXT = "NoAudioText"; 
        ...
        public void Open()
        {
            ...
            //{
            //UIConfirmationMenu.instance.Show("You cannot log while viewing history.", new UIConfirmationMenu.ConfirmationButton("Okay", null));
            //return;
            //} 
        ...
        // Add this method to retrieve logs
        ...
        private void CreateLog(HistoryState state)
        {
            ...
            log.dialogueAudio = log.container.transform.Find(AUDIO_FROM_TEXT).GetComponent<AudioSource>();  
            ...
            // Assign the upcoming voice to this new log
            HistoryManager.instance.AssignUpcomingVoiceToLog(log);

            // Check if there is an audio clip assigned to the AudioSource
            if (log.dialogueAudio != null && log.dialogueAudio.clip == null)
            {
                // No sound assigned, make the button invisible
                log.container.transform.Find(AUDIO_FROM_TEXT).gameObject.SetActive(false);
                log.container.transform.Find(NO_AUDIO_TEXT).gameObject.SetActive(true); 
            }
            else
            {
                // There is sound, keep the button visible
                log.container.transform.Find(AUDIO_FROM_TEXT).gameObject.SetActive(true);
                log.container.transform.Find(NO_AUDIO_TEXT).gameObject.SetActive(false); 
            }
        }
        ...
        // Method to be attached to the "AudioFromText" button
        public void PlayAudioFromText(AudioSource audioSource)
        {
            ...
            {
                Debug.Log("No audio clip is assigned to the AudioSource."); 
            }
        } 
