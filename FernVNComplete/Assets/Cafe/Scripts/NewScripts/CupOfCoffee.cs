...
public class CupOfCoffee : MonoBehaviour
{
    ...
    public GameObject whippedCoffee; 
    ...
    private void CheckReturnToMachine()
    {
        ...
        if (!isSnapped && coffeeMachineCom != null)
        {
            ...
            //coffeeMachineCom.CheckForDuplicateCups(gameObject);
        }
    }

    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("WhipCream")) 
        {
            OnDestroyed?.Invoke(); // Invoke the OnDestroyed event
            QuoteMenu.Instance.StartCoroutine(QuoteMenu.Instance.HideButtonsTemporarily(QuoteMenu.Instance.selectedSnapPointIndex, 2f)); // Hide buttons temporarily
            Destroy(this.gameObject);
            Instantiate(whippedCoffee, transform.position + new Vector3(0, -0.5f, 0), transform.rotation);
        }
    }
}
