...
public class CoffeeBeans : MonoBehaviour
{
    ...
    public Vector2 coffeeBeansSlots;
    public float speed;
    ...
    void Update()
    {
        if (shouldMove)
        {
            ...
            transform.position = Vector2.MoveTowards(transform.position, coffeeBeansSlots, speed * Time.deltaTime);

            // Check if the object has reached the target slot
            if (Vector2.Distance(transform.position, coffeeBeansSlots) < 0.01f)
            {
                transform.position = coffeeBeansSlots;
                shouldMove = false;
    ...
    void OnDestroy()
    {
        CoffeeBeanJar coffeeBeanJar = FindObjectOfType<CoffeeBeanJar>();
        if (coffeeBeanJar != null)
        {
            coffeeBeanJar.CoffeeBeansDestroyed();
        }
    }
}
